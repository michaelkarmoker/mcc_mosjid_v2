...
class CustomButton extends StatelessWidget {
  ...
  CustomButton({required this.onPressed,this.color, required this.buttonText,  this.margin, this.width, this.height,
    this.fontSize, this.radius=5, this.icon, this.btnColor, });
    ...
    return Center(child: SizedBox(width: width != null ? width : Dimensions.WEB_MAX_WIDTH,height: height!=null?height:Dimensions.BUTTON_HEIGHT, child:Container(
      ...
      child:Material(
          ...
          child: InkWell(
            onTap: onPressed,
            child: Center(
                child: Text(buttonText,textAlign: TextAlign.center,style: TextStyle(color:color??Colors.white,fontWeight: FontWeight.w600,fontSize: fontSize??Dimensions.fontSizeExtraLarge),)),
          )),
    )
    ));
  }
